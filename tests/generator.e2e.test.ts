...
describe.each(generators.filter((g) => !g.skip))("DataGenerator: $name", ({
  create,
}) => {
  ...
  it("should resume sequences when resumeSequences is true", {
    timeout: 30_000,
  }, async () => {
    await generator.truncateTable(testTable.name);
  ...
  it("should handle BIGINT sequence starting at 2^31 without overflow", {
    timeout: 30_000,
  }, async () => {
  ...
  it("should apply mutate transformation with probability", {
    timeout: 30_000,
  }, async () => {
  ...
  it("should apply mutate transformation with multiple random operations", {
    timeout: 30_000,
  }, async () => {
  ...
  it("should apply lookups before template/mutate in same batch (ClickHouse behavior)", {
    timeout: 30_000,
  }, async () => {
  ...
  it("should apply swap transformation with probability", {
    timeout: 30_000,
  }, async () => {
  ...
  it("should run a complete scenario with runScenario", {
    timeout: 30_000,
  }, async () => {
    const scenario = {
      name: "Test scenario",
  ...
  it("should run a multi-table scenario with runScenario", {
    timeout: 30_000,
  }, async () => {
    const scenario = {
  ...
  it("should run a scenario with transform-only steps", async () => {
    ...
    // First step: generate
    // biome-ignore lint/style/noNonNullAssertion: steps[0] exists per assertion above
    ...
    // biome-ignore lint/style/noNonNullAssertion: steps[1] exists per assertion above
